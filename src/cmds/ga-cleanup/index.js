...
module.exports = async(obj = {})=>{
  try{
    ...
    let leader = opt.leader?.value, exclude_gl = opt.exclude_gl?.value || false, sort = opt.sort?.value || defaultSettings?.sort || 'rate', singleUnit = opt.single_unit?.value || false
    ...
    let pipeline = [], countPipeline = [], searchString = '^cleanup-', collection = `gaCounter-${season}`
