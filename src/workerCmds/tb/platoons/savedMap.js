...
const GetNumUnits = (member = [], baseId, relicTier = 0, rarity = 1)=>{
  try{
    return +member.filter(x=>x?.roster && x.roster[baseId] && x.roster[baseId].rarity >= rarity && (x.roster[baseId].combatType === 2 || x.roster[baseId].relicTier >= relicTier)).length
...
module.exports = (guild = [], pDefinition = [], platoonIds = [], tbDay = 1)=>{
  try{
    ...
    for(let i in pDef){
      if(!pDef[i].exclude && !pDef[i].prefilled){
        ...
        for(let s in squads){
          ...
          const squadConfig =  GetSquadConfig(platoonIds, pDef[i].id, squads[s].num)
          ...
          if(!tempSquad.exclude && !tempSquad.prefilled){
            const unitCounts =  GetUnitCount(squads[s].units)
            for(let u in unitCounts){
              ...
              let avaliableUnits = units[unitCounts[u].baseId].filter(x=>x.rarity >= unitCounts[u].rarity && (x.combatType === 2 || x.relicTier >= unitCounts[u].unitRelicTier - 2))
              ...
              if(unitCounts[u].count > 0 && +avaliableUnits?.length > 0 && +avaliableUnits?.length >= unitCounts[u].count){
                ...
                for(let m = 0;m<unitCounts[u].count;m++){
                  tempSquad.units.push({nameKey: unitCounts[u].nameKey, baseId: unitCounts[u].baseId, available: +avaliableUnits?.length, count: GetNumUnits(guild, unitCounts[u].baseId, unitCounts[u].unitRelicTier - 2, unitCounts[u].rarity)})
                }
              }
            }
