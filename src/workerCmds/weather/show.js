'use strict'
const { mongo, ButtonPick, GetOptValue, ReplyButton, ReplyMsg } = require('helpers')
...
module.exports = async(obj = {}, opt = [])=>{
  try{
    ...
    let city = GetOptValue(opt, 'city')?.toString().trim().toLowerCase()
    let state = GetOptValue(opt, 'state')?.toString().trim().toUpperCase()
    let country = GetOptValue(opt, 'country')?.toString().trim().toUpperCase()
    if(opt.find(x=>x.name == 'set')) setDefault = 1
    if(city){
      ...
      if(obj.confirm?.cityId) cityObj = (await mongo.find('weatherCities', {_id: obj.confirm.cityId}))[0]
    ...
    if(cityObj && cityObj.coord && cityObj.coord.lon && cityObj.coord.lat){
      await ReplyButton(obj, 'Getting Weather for **'+cityObj.name+'**'+(cityObj.state ? ', **'+cityObj.state+'**':'')+(cityObj.country ? ' in **'+cityObj.country+'**':''))
