...
module.exports = async(opts = {})=>{
  try{
    ...
    if(!guildId) return
    let guild = await getGuild({ guildId: guildId, collection: 'guildCache', project: opts.guildProject })
    ...
    let players = await getGuildMembers(guildId, guild.member, dataCount, opts.playerProject)
    if(!players) return
