'use strict'
const { DeepCopy, GetOptValue } = require('helpers')
...
module.exports = (guild = [], pDef, opt=[])=>{
  try{
    ...
    pDef.platoons = pDef.platoons.filter(x=>x.phase === 'P'+round)
    if(conflict !== 'all') pDef.platoons = pDef.platoons.filter(x=>x.conflict === conflict)
    pDef.platoons = sorter([{column: 'sort', order: 'ascending'}], pDef.platoons)
    for(let i in pDef.platoons){
      let tempPlatoon = DeepCopy(pDef.platoons[i])
      tempPlatoon.squads = []
      tempPlatoon.points = 0
      let playerUnitCount = {}
      let squads = (pDef.platoons[i].squads)
      for(let s in squads){
        let cgIsDumb = true
        if(+s === 0){
      ...
      if(tempPlatoon.squads?.length > 0){
        ...
        tempPlatoon.points = tempPlatoon.points
        tempPlatoon.totalPoints = tempPlatoon.totalPoints
