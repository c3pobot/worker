...
module.exports = async(obj = {})=>{
  try{
    ...
    if(!roleId) roleId = await GetOptValue(obj.data?.options, 'role')
    if(dId){
    ...
    if(roleId){
      ...
      const roles = await DiscordQuery('guilds/'+obj.guild_id+'/roles')
    ...
    if(guild?.selfassignroles?.length > 0 && roleId && roleName){
      ...
      if(guild.selfassignroles.filter(x=>x.id === roleId).length > 0){
        ...
        botPerms = await GetBotPerms(obj.guild_id)
      }
    }
    ...
    if(hasRolePerm){
      ...
      let status = await DiscordQuery('guilds/'+obj.guild_id+'/members/'+dId+'/roles/'+roleId, 'DELETE')
