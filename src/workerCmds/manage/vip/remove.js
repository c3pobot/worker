'use strict'
const { mongo, ConfirmButton, GetOptValue, ReplyMsg } = require('helpers')
module.exports = async(obj = {}, opt = [])=>{
  try{
    ...
    if(dId){
      ...
      if(!deleteConfirm){
        await ConfirmButton(obj, 'Are you sure you want to remove vip member '+(usr ? '**@'+usr+'**':'with discord Id **'+dId+'**')+'?')
        return
      }else{
