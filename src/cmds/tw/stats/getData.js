...
module.exports = async(guildData = {})=>{
  ...
  for(let i in guildData.homeGuild?.conflictStatus){
    ...
    if(zoneName){
      ...
      if(res.homeDefense[zoneName]?.squad) getDefense(res.homeDefense[zoneName].squad, guildData.homeGuild.conflictStatus[i]?.warSquad)
    }
  ...
  for(let i in guildData.awayGuild?.conflictStatus){
    ...
    if(zoneName){
      ...
      if(res.awayDefense[zoneName]?.squad) getDefense(res.awayDefense[zoneName].squad, guildData.awayGuild.conflictStatus[i]?.warSquad)
    }
