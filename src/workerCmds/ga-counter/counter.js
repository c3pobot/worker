...
module.exports = async(obj = {}, opt = [], mode = '5v5')=>{
  try{
    let msg2send = {content: 'you did not specify a valid unit'}, webHTML, webImg, tempSquads, countSquads, squads = [], defaultSettings, skip = 0, method = 'PATCH'
    ...
    for(let i in opt){
      if(opt[i]?.name?.startsWith('unit')){
        ...
        if(baseId && configMaps.UnitMap[baseId]){
          if(!str) str = ' with'
          str += ' '+configMaps.UnitMap[baseId].nameKey
    ...
    if(tempSquads?.length > 0){
      ...
      if(squads?.length > 0){
        ...
        info.header = 'GAC Season '+countSquads?._id+' '+countSquads?.mode+' '+(configMaps.UnitMap[leader]?.nameKey || leader)+' counters in '+countSquads?.total+' battles'
      }
    }
