...
const GetImg = require('src/cmds/faction/basic/getImg')
module.exports = async(obj, opt = [])=>{
  try{
    let msg2send = {content: 'You do not have a allyCode linked to discord Id'}, faction, fInfo, eObj, gaInfo, webUnits, webData, factionImage
    ...
    if(gaInfo?.currentEnemy){
      ...
      combatType = HP.GetOptValue(opt, 'option', 1)
      if(combatType) combatType = +combatType
    }
    if(faction){
