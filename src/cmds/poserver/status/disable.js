...
module.exports = async(obj = {}, opt = {})=>{
  ...
  if(obj.confirm?.response == 'yes'){
    await mongo.set('payoutServers', { _id: shard._id }, { status: 0 })
    shard.status = 0
  ...
  let msg2send = { content: `are you sure you want to disable shard ${shard._id} on ${guild?.name}?`, components: [] }
  let actionRow = { type: 1, components: [] }
  actionRow.push({
    type: 2,
    label: 'Yes',
    style: 3,
    custom_id: JSON.stringify({ id: obj.id, dId: obj.message?.user?.id, response: 'yes', subCmd: 'disable' })
  })
  actionRow.push({
    type: 2,
    label: 'no',
