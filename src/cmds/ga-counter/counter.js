...
module.exports = async(obj = {}, opt = [], mode = '5v5')=>{
  ...
  let season = getOptValue(opt, 'season')
  let exclude_gl = getOptValue(opt, 'exclude_gl')
  let sort = getOptValue(opt, 'sort', defaultSettings?.sort || 'rate')
  let minBattles = getOptValue(opt, 'battles', defaultSettings?.battles || 2)
  let battleLimit = getOptValue(opt, 'limit', defaultSettings?.limit || 10)
  let league = getOptValue(opt, 'league', 'KYBER')
