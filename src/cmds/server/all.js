...
module.exports = async(obj = {}, opt = [])=>{
  try{
    ...
    const tempObj = await BotSocket.call('getAllServerStats', {})
    if(tempObj?.guilds) guildCount = tempObj.guilds
    if(tempObj?.users) userCount = tempObj.users
    const embedMsg = {
      color: 15844367,
    ...
    const basicServers = await mongo.count('discordServer', {basicStatus: 1})
