...
module.exports = async(obj, shard, opt = [])=>{
  try{
    let msg2send = {content: 'There are no squad lead aliases for this server', components: []}, startIndex = 0
    ...
    if(shard && shard.alias && shard.alias.length > 0){
      msg2send.content = null
      const alias = await sorter([{column: 'alias', order: 'ascending'}], shard.alias)
      const embedMsg = {
        color: 15844367,
