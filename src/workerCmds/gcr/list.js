...
module.exports = async(obj = {}, opt = [])=>{
  try{
    let msg2send = {content: 'There are no global custom reactions'}, crQuery, cr = []
    let crQuery = await GetOptValue(opt, 'query')
    if(crQuery) crQuery = crQuery.toString().toLowerCase()
    const localCR = (await mongo.find('reactions', {_id: 'global'}))[0]
