...
const { FindUnit, GetAllyCodeObj, GetOptValue, ModifyUnit, ReplyButton } = require('helpers')
const swgohClient = require('swgohClient')
module.exports = async(obj = {}, opt = [])=>{
  try{
    let allyCode, baseId, dObj, pObj, webHTML, webImg, msg2send = {content: 'error finding the requested unit'}
    let gLevel = 13, rLevel = botSettings?.map?.maxRelic || 10
    if(obj.confirm){
    ...
    if(pObj?.roster[baseId]){
      ...
      if(modifyUnit){
        ...
        //webHTML = await getHTML({player: pObj.name, updated: pObj.updated, unit1: pObj?.roster[baseId], unit2: modifiedUnit})
        mongo.set('webTemp', {_id: 'unitgear'}, {data: {player: pObj.name, updated: pObj.updated, unit1: pObj?.roster[baseId], unit2: modifiedUnit}})
      }
    }
