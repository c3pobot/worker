...
module.exports = async(obj, opt = [])=>{
  try{
    let allyCode, pObj, msg2send = {content: 'You do not have allycode linked to discordId'}, statInfo = {}
    const allyObj = await HP.GetPlayerAC(obj, opt)
    let sort = HP.GetOptValue(opt, 'sort') || 'stat'
    ...
    if(pObj && pObj.rosterUnit && pObj.rosterUnit.length > 0){
      ...
      let units = await FT.FormatPlayerStats(pObj.rosterUnit.filter(x=>x.combatType === 1), statInfo, sort)
      if(units?.length > 0){
